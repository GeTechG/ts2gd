...
export const parseCallExpression = (node: ts.CallExpression, props: ParseState): ParseNodeType => {
    ...
    if (node.expression.kind === SyntaxKind.PropertyAccessExpression) {
        ...
        if (
            ...
            if (functionName === "add" || functionName === "sub" || functionName === "mul" || functionName === "div") {
                const libFunctionName = (functionName + "_vec_lib") as LibraryFunctionName;
    ...
    if (expression.kind === SyntaxKind.PropertyAccessExpression) {
        ...
        if (
            isDictionary(props.program.getTypeChecker().getTypeAtLocation(propAccess.expression)) &&
    ...
    const isExpressionArrowFunction =
        decls && decls[0].kind === SyntaxKind.ArrowFunction && decls[0].getSourceFile() === node.getSourceFile();
    ...
    let result = combine({
        ...
        parsedObjs: (parsedExpr, ...parsedArgs) => {
            ...
            if (parsedExpr.content.endsWith(".emit")) {
                ...
                if (node.expression.kind === SyntaxKind.PropertyAccessExpression) {
                    ...
                    if (pae.expression.kind === SyntaxKind.PropertyAccessExpression) {
                        ...
                        parsedStringArgs = [`"${signalName}"`, ...parsedArgs.map((arg) => arg.content)];
                        parsedExpr = {
            ...
            if (parsedExpr.content.endsWith(".connect")) {
                if (expression.kind === SyntaxKind.PropertyAccessExpression) {
                    ...
                    if (pae.kind === SyntaxKind.PropertyAccessExpression) {
                        ...
                        if (!parsedArgs[0]) {
                            addError({
                                description: "Missing arrow function argument in signal connect invocation.",
                            ...
                            if (!arrowFunctionObj) {
                                ...
                                const { capturedScopeObject } = getCapturedScope(arrowFunctionObj.node, props);

                                parsedStringArgs = [`"${signalName}"`, "self", `"${arrowFunctionObj.name}"`, `[${capturedScopeObject}]`];
                                ...
                                parsedExpr = {
                                    content: parsedExpr.content.substring(0, firstDot) + parsedExpr.content.substring(secondDot),
                                };
                            }
                        }
                    }
            ...
            if (parsedExpr.content.endsWith(".rpc") || parsedExpr.content.endsWith(".rpc_id")) {
                if (expression.kind === SyntaxKind.PropertyAccessExpression) {
                    const pae = expression as ts.PropertyAccessExpression;
                    ...
                    if (pae.expression.kind === SyntaxKind.PropertyAccessExpression) {
                        ...
                        const expressionWithoutRpcName =
                            parsedExpr.content.substring(0, firstDot) + parsedExpr.content.substring(secondDot);
                        ...
                        if (isRpcId) {
                            ...
                            parsedStringArgs = [`"${rpcFunctionName}"`, ...parsedArgs.map((arg) => arg.content)];
                        }
            ...
            const isFunctionObject =
                !isFromLib &&
                (calledExpressionType === ts.SyntaxKind.Parameter || calledExpressionType === ts.SyntaxKind.VariableDeclaration);
