...
export const parseCallExpression = (
    ...
    if (expression.kind === SyntaxKind.PropertyAccessExpression) {
        ...
        if (
            isDictionary(
                props.program
                    .getTypeChecker()
                    .getTypeAtLocation(propAccess.expression),
            ) &&
    ...
    let result = combine({
        ...
        parsedObjs: (parsedExpr, ...parsedArgs) => {
            let parsedStringArgs: string[] = parsedArgs.map(
                (arg) => arg.content,
            );
            ...
            if (parsedExpr.content.endsWith(".emit")) {
                ...
                let signalName = parsedExpr.content.slice(
                    firstDot + 1,
                    secondDot,
                );
                ...
                if (
                    ...
                    if (
                        ...
                        const pae2 =
                            pae.expression as ts.PropertyAccessExpression;
                    ...
                    if (pae.kind === SyntaxKind.PropertyAccessExpression) {
                        ...
                        if (!parsedArgs[0]) {
                            ...
                            if (!arrowFunctionObj) {
                                ...
                                const secondDot =
                                    parsedExpr.content.lastIndexOf(".");
                                const firstDot = parsedExpr.content.lastIndexOf(
                                    ".",
                                    secondDot - 1,
                                );
                ...
                nullCoalesce = [
                    {
                        ...
                        line: `var ${newName} = ${
                            ...
                            needsExplicitSelfArg
                                ? parsedExpr.content + "[2], "
                                : ""
                        }${parsedStringArgs}) if ${
                            parsedExpr.content
                        } != null else null`,
            ...
            if (isFunctionObject) {
                return `${
                    parsedExpr.content
                }[0].call_func(${parsedStringArgs.join(", ")})`;
            } else {
