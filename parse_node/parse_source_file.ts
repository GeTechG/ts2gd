 ...
 */
const getClassDeclarationHeader = (node: ts.ClassDeclaration | ts.ClassExpression, props: ParseState) => {
    ...
    const isTool = !!ts.getDecorators(node)?.find((dec) => dec.expression.getText() === "tool");

    return `${isTool ? "tool\n" : ""}${extendsFrom ? `extends ${extendsFrom}` : ""}
${!node.name ? "" : `class_name ${node.name.getText()}\n`}`;
};
...
export const parseSourceFile = (node: ts.SourceFile, props: ParseState): ParseNodeType => {
    ...
    const sourceInfo = props.project.sourceFiles().find((file) => file.fsPath === node.fileName);
    ...
    const allClasses = statements.filter(
        (statement) =>
            ...
            ((ts.canHaveModifiers(statement) ? ts.getModifiers(statement) : undefined) ?? []).filter((m) => m.getText() === "declare")
                .length === 0,
    ...
    if (allClasses.length === 0) {
        addError({
            ...
            location: node,
            description: "Every file must have one class in it, but this file doesn't have any.",
    ...
    for (const statement of statements) {
        if (statement.kind !== SyntaxKind.ClassDeclaration && statement.kind !== SyntaxKind.ClassExpression) {
        ...
        if (!(ts.canHaveModifiers(statement) ? ts.getModifiers(statement) : undefined)?.map((m) => m.getText()).includes("declare")) {
            ...
            const className = classDecl.name?.text ?? props.sourceFileAsset.name;
            ...
            parsedClassDeclarations.push({
                fileName: props.sourceFileAsset.gdContainingDirectory + className + ".gd",
        ...
        for (const lf of parsedStatement.hoistedLibraryFunctions ?? []) {
            hoistedLibraryFunctionDefinitions += LibraryFunctions[lf].definition("__" + LibraryFunctions[lf].name) + "\n";
        }
